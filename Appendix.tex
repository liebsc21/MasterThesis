...
\begin{scope}[shift={(7,-4)}]
	...
	\path[line width=0.8pt,>-] (0.25,-0.3) edge [out=180, in=0](1.25,-0.3);
\end{scope}
...
\begin{scope}[shift={(8,-10.5)}]
	\node at (-1.5,0.7) {\circled{10a}};
	\draw[ghost] (0,0)--(45:1);
	\draw[ghostbar] (0,0)--(-45:1);
	\draw[gluon] (0,0)--(180:1);
	\node at (45:1.4) {$\overline{c}^a(-p_1)$};
	\node at (-45:1.4) {$c^c$};
	\node at (180:1.3) {$G_\mu^b$};
	\node at (3,0) {$\hat{=} -g_s f^{abc}p_1^\mu$};
\end{scope}
...
In the Feynman diagrams of the vertices all momenta flow towards the vertex, i.e. in diagram 9 and 10a: $-p_1$ flows towards the vertex.\newline
...
\begin{scope}[shift={(0,-5)}]
	...
	\node at (3,0) {$\hat{=} \ ig_s^2 (f^{abc}f^{ade} + f^{abe}f^{adc})$};
\end{scope}
\end{tikzpicture}
