...
&D^\alpha := \epsilon^{\alpha\beta} D_\beta = -\frac{\partial}{\partial \theta_\alpha} + i(\overline{\theta} \overline{\sigma}^\mu)^\alpha \partial_\mu && \mathcal{D}_{\dot{\alpha}} := \epsilon_{\dot{\alpha}\dot{\beta}} \mathcal{D}^{\dot{\beta}} = -\frac{\partial}{\partial \overline{\theta}^{\dot{\alpha}}} + i(\sigma^\mu\theta)^{\dot{\alpha}} \partial_\mu.
\end{align}
...
The indices $A,B = L,R$ label the right/left-"handedness" of the squarks. The indices $i,j=1,2,3$ are the color indices in the (anti)fundamental representation where $a,b,c,\hdots = 1,\hdots,8$ are the color indices of the adjoint representation.}\label{fig:firstFeynmanRules}
\end{center}
\end{figure}
\newpage
...
B_1(0,0,0) &= 0
\end{align}
The scaleless integrals are defined to be zero. The parameter $t$ is given by $\frac{m_2^2}{m_1^2}$. As can be seen from \ref{eq:B1} $B_1$ is in contrast to $B_0$ not symmetric in its masses but it can be shown [Romao] that
\begin{align}
