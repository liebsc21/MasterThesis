...
\end{tabular}
\caption{$K$-factors of the process $pp \to \tilde{u}_L\tilde{u}_R$  in the MRSSM for a selected set of masses. The center-of-mass energy is $\sqrt{S} = \unit[13]{TeV}$ and the pseudoscalar mass is fixed to $m_{\mathrm{\sigma}} = \unit[5000]{GeV}$.}\label{tab:Modelfille}
\end{center}
\end{table}
...
\end{lstlisting}
the $s$-channel gluinos were omitted, with the addendum \texttt{\$\$go} as \texttt{MadGraph5} does not deal with divergences coming from the gluino in the on-shell limit. But as already discussed, these contributions are only of sub percent order. In fact, comparing the results from \texttt{MadGraph5} with the ones obtain by using the  program \texttt{Prospino2}, which does include these contributions, shows good agreement of the next-to leading order cross sections of $pp \to \tilde{u}_L \tilde{u}_R$:
\begin{align}
...
\begin{figure}[!htpb]
\begin{center}
\includegraphics[scale=.55]{figures/MSSM_uu_susu_Kfactors_msg=2000GeV.png}
\includegraphics[scale=.55]{figures/MSSM_uu_susu_Kfactors_msq=1000GeV.png}
...
\end{tabular}
\caption{$K$-factors of the process $pp \to \tilde{u}_L\tilde{u}_R$ and $pp \to \tilde{u}\tilde{u}$, where $\tilde{u}\tilde{u} \in \left\{ \tilde{u}_L\tilde{u}_R, \tilde{u}_L\tilde{u}_L, \tilde{u}_R\tilde{u}_R \right\}$, in the MSSM for a selected set of masses. The center-of-mass energy is $\sqrt{S} = \unit[13]{TeV}$.}\label{tab:Modelfille}
\end{center}
\end{table}
