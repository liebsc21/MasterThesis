...
The computation of the real correction has been done by Wojciech Kotlarski, another member of the group. Comment on how real corrections are processed.\\
\begin{figure}[!htbp]
\begin{center}
\begin{tikzpicture}
	...
	\node [block1a] at (-1,-1.5) (notebookV) {\texttt{FeynArts} $\to \mathcal{M}^{\mathrm{1L}}$ \newline \texttt{FormCalc} $\to 2 \sum \Re(\mathcal{M}^{\mathrm{B}}\mathcal{M}^{\mathrm{1L}\ast})$};
