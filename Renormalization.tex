...
\end{align}
is the supersymmetry restoring renormalization constant. Note the sign within this definition in comparison to eq. \eqref{eq:GammaCtRestore}. The only point where particular care is required is the coupling: The  gauge coupling $g_s$ and the Yukawa coupling $\hat{g}_s$ receive different supersymmetry restoring counterterms:
\begin{align}
...
\begin{align}
Z = 1 + \delta Z \hspace{3cm} o_{\mathrm{bare}} = o + \delta o\label{eq:renconst}
\end{align}
where $Z$ and $o_{\mathrm{bare}}$ are defined by the multiplicative renormalization transformation, introduced in eq. \eqref{eq:fieldtrafo} and \eqref{eq:parametertrafo}. The field and mass renormalization constants have been calculated in DREG in the on-shell scheme. The advantage of this is that when turning to the cross section no modification of the Green function to the S-matrix element has to be done. This is discussed in more detail in section \ref{sec:LSZ}.\\
...
From the Lagrangian in eq. \eqref{eq:L_RSQCD} one can see that the couplings of the two particles are quite distinct. This is reflected by different field renormalization constants of the left- and right-handed part of the gluino (see eq. \eqref{eq:fieldtrafo}).
\begin{figure}[!htbp]
\begin{center}
\begin{tikzpicture}[line width=1.5 pt, scale=1.3]
...
\end{scope}
\end{tikzpicture}
\caption{Diagrammatic contributions to the self-energy of the gluino at 1-loop level}\label{fig:GluinoSE}
\end{center}
\end{figure}
As for the quarks, the fermion (and momentum) flow in a diagram is from the right to the left. The Feynman diagrams for the gluino self-energy are shown in \ref{fig:GluinoSE}. The one-particle irreducible Green function is given by
\begin{align}
...
&-4 C(A) m_{\tilde{g}}^2 \frac{\partial}{\partial p^2} \left.\left( -B_0(p^2,0,m_{\tilde{g}}^2) + B_1(p^2,0,m_{\tilde{g}}^2) \right)\right]_{p^2=m_{\tilde{g}}^2}
\end{align}
and 
\begin{align}
...
\end{center}
\end{figure}
This can be seen when writing down the matrix amplitudes for the counterterm Feynman diagrams shown in fig. \ref{fig:CounterTerm}:
\begin{align}
...
The quark self-energy has two contributions which are shown in figure \ref{fig:QuarkSelfEnergy}. The first one corresponds to light particles and the second one to heavy particles: $i\Gamma^{\mathrm{1L}}_{q_i\overline{q}_j} = i\Gamma^{\mathrm{1L,light}}_{q_i\overline{q}_j} + i\Gamma^{\mathrm{1L,heavy}}_{q_i\overline{q}_j}$. For light particles only the UV-divergent\footnote{In contrast to the MS-scheme in the $\overline{\mathrm{MS}}$-scheme not only the pure ultraviolet divergence but also two additional transcendent numbers are subtracted. It is therefore common to define $\Delta_\epsilon = \frac{1}{\epsilon} -\gamma_{\mathrm{E}} + \ln 4\pi$.} part is kept. 
...
\end{center}
\end{figure}
As for the quark self-energy, there are again contributions to the self-energy of the gluon originating from light and heavy particles. Again these are differently dealt with. The ultraviolet divergent one-particle irreducible Green function is given by
\begin{align}
...
\begin{tikzpicture}[line width=1.5 pt, scale=1.3]
	...
	\draw[fermion](-45:0.5)--(-45:1.3);
	\node at (1.8,0) {=};
\begin{scope}[shift={(3.5,0)}]
	\draw[gluon](180:1.3)--(180:0.5);
	\draw[fermionbar](180:0.5)--(45:0.5);
	\node at (0,0.5) {$q$};
	\draw[fermion](180:0.5)--(-45:0.5);
	\node at (0,-0.5) {$q$};
...
&= ig_s T^a_{ij} \gamma^\mu \frac{g_s^2}{16\pi^2} 2C(F)B_1(0,m_{\tilde{g}}^2,m_{\tilde{q}}^2).\label{eq:something}
\end{align}
The argument $p_i^2=0$ of the vertex function denotes that all external particles are taken on shell, i.e. at zero momentum.\\
In the second line of eq. \eqref{eq:something} identities from section \ref{sec:Passarino} have been used. The renormalization condition for the counterterm of the gauge coupling $\delta g_s$ reads
\begin{align}
...
Writing down the action of a theory in $D$ dimensions, one needs to introduce an energy scale $\mu$ in order to keep the action dimensionless. For $\mu$ not being a physical parameter it can be absorbed into the fields and parameters. To this end, one defines the bare coupling
\begin{align}
...
The diagrams from which the supersymmetry restoring contributions to the $q\overline{q}G$ vertex come from are shown in figure \ref{fig:GaugeCouplingCorrectionL} and evaluate to
\begin{align}
...
The supersymmetry restoring corrections to the $\tilde{q}_L \overline{q}\tilde{g}$ vertex originate from the diagram in figure \ref{fig:YukawaCT}.
\begin{figure}[!htbp]
\begin{center}
\begin{tikzpicture}[line width=1.5 pt, scale=1.3]
	\draw[gluon](180:1.3)--(180:0.5);
...
\end{align}
In short, this means that the gauge coupling $g_s$ is renormalized with $\delta g_s$ given in eq. \eqref{eq:deltaGs} and the Yukawa coupling $\hat{g}_s$ is renormalized with $\delta \hat{g}_s = \delta g_s + \delta g_s^{\mathrm{restore}}$.\\
...
To check quickly for UV-finiteness of a Greenfunction or a physical observable it is useful to have the extracted UV-divergences of the renormalization constants at hand. How these are obtained is described in section \ref{sec:Passarino}. They evaluate to:
\begin{align}
...
This section explains how the renormalization of the matrix element of squark production is implemented in the framework of the automated calculation and gives intermediate results in form of the renormalized matrix element.
...
$\epsilon^{-1}$ & $\epsilon^{-1}$ & (\texttt{SetLambda} $\to -1$, \texttt{Dminus4} $\to 0$) \\
  & $+\mathcal{O}(\epsilon)\cdot\epsilon^{-2}$ & $+$(\texttt{SetLambda} $\to -2$, \texttt{Dminus4} $\to -2$) \\
...
\caption{The figure at the bottom shows the absolute squared matrix element of $uu \to \tilde{u}_L\tilde{u}_R$ in the MRSSM at tree-level with $\alpha_s(m_Z) = 0.135$ from the leading order parton density functions $\mathtt{MMHT2014}$ and $\alpha_s(m_Z) = 0.12$ from the next-to-leading order parton density function as well as the contribution from the virtual corrections to it.\newline 
The figure at the top shows the different contributions to the virtual corrections as well as their sum. The used parameters are given under fig. \ref{fig:MatrixElementPoles}.}\label{fig:MatrixElement}
\end{center}
\end{figure}