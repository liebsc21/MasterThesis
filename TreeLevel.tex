...
In this chapter, the production of strongly interacting supersymmetric particles\footnote{The production of sgluons is excluded from the analysis for their mass is chosen to be too large to significantly produce them. See \cite{Choi:2008ub, GoncalvesNetto:2012nt} for a calculation of sgluon cross section.} in the MRSSM is considered. The various processes and their associated cross section are compared to their analogues in the MSSM.\\
Section \ref{sec:PartProcesses} will give analytic results for the parton reaction before the next section describes how the hadronic cross section is obtained. Section ??? presents the hadronic cross section of all considered processes. In particular, squark production is considered more closely. This includes the statement of uncertainties of the calculation.
...
In contrast to the MSSM no statistical factor of $\frac{1}{2}$ is taken into account when turning from $|\mathcal{M}|^2$ to $\sigma$. This is because gluino and antigluino are distinguishable particles. Still in comparison to the MSSM cross section \cite[p.9]{Beenakker:1996ch} only the first line in \eqref{eq:qqbar_to_sgsgbar} is doubled up as the other two lines originate from an $t$ or $u$ channel squark which occurs in only one instead of two chiralities. Furthermore an interference term from the $t$ and $u$ channel diagram which occurs in the MSSM is absent in the MRSSM.
...
\subsection{Results for Squark and Gluino Production at Tree-Level}
REFER TO Kribs, Martin: SNOWMASS WHITEPAPER\\
REFER TO Vernoica Sanz: How many SUpersymmetries?\\
\\
This section presents hadronic tree-level or (Born) cross sections of squark and gluino production for two colliding protons in both, the MSSM and the MRSSM.\\
Note that in the following all possible production channels are taken into account. This is, for squark and squark-gluino production, also the charged conjugated processes, i.e. $\tilde{q}^\dagger\tilde{q}^\dagger$ and $\tilde{q}^\dagger\overline{\tilde{g}}$, are included. In addition, squark production counts thoses processes twice which involve flavor unlike quarks, e.g. $ud \to \tilde{u}\tilde{d}$ is counted twice as the $d$-quark may come from either proton. This logic applies also to squark-antisquark production through a quark-antiquark pair.\\
The calculations have been performed by a $\texttt{C++}$ code which uses a library containing the absolute squared amplitudes for the processes in question. Note that the author has opted for the absolute squared amplitudes and not the partonic cross sections because this allows also for the calculation of partial differential cross section like the one shown in fig. ??? at the end of the section.\\
...
\end{center}
\end{figure}
The contour plots \ref{fig:ContourTree} underline the already announced difference between squark production in the MRSSM and the MSSM. While the cross sections for small gluino masses resemble each other closely, the cross section in the MRSSM gets increasingly suppressed with increasing gluino mass. 
\begin{figure}[!htpb]
\begin{center}
...
$m_{\tilde{g}}$ \textbackslash $m_{\tilde{q}}$ & $\unit[500]{GeV}$ & $\unit[1000]{GeV}$ & $\unit[1500]{GeV}$\\
\hline
$\unit[1000]{GeV}$ & $934.9_{\pm 27.2\%\ignore{735}\pm 2.1\% \ignore{915}}^{\pm 30.0\% \ignore{1213}\pm 1.2\% \ignore{946}}$ & $102.1_{\pm 29.2\% \ignore{79.0}\pm 2.1\%\ignore{100}}^{\pm 31.2\% \ignore{134}\pm 1.9\% \ignore{104}}$ & $13.12_{\pm 31.6\% \ignore{9.97}\pm 1.7\% \ignore{12.9}}^{\pm 34.1\% \ignore{17.6}\pm 2.1\% \ignore{13.4}}$\\
$\unit[1500]{GeV}$ & $360.9_{\pm 30.0\% \ignore{282}\pm 2.2\% \ignore{353}}^{\pm 30.5\% \ignore{471}\pm 1.1\% \ignore{365}}$ & $50.52_{\pm 29.5\% \ignore{39.0}\pm 1.9\% \ignore{49.6}}^{\pm 32.4\% \ignore{66.9}\pm 1.7\% \ignore{51.4}}$ & $7.712_{\pm 31.8\% \ignore{5.85}\pm 2.8\% \ignore{7.5}}^{\pm 34.9\% \ignore{10.4}\pm 2.4\% \ignore{7.9}}$\\
$\unit[2000]{GeV}$ & $165.5_{\pm 28.2\% \ignore{129}\pm 2.1\% \ignore{162}}^{\pm 30.5\% \ignore{216}\pm 0.9\% \ignore{167}}$ & $26.97_{\pm 29.8\%\ignore{20.8}\pm 2.1\% \ignore{26.4}}^{\pm 32.6\%\ignore{35.8} \pm 1.5\% \ignore{27.4}}$ & $4.625_{\pm 31.8\% \ignore{3.51}\pm 1.6\% \ignore{4.55}}^{\pm 34.5\% \ignore{6.22}\pm 1.8\% \ignore{4.71}}$\\
\end{tabular}
\caption{MRSSM The uncertainty of the numerical integration is lower than one permille.}
\end{center}
\end{table}
...
$m_{\tilde{g}}$ \textbackslash $m_{\tilde{q}}$ & $\unit[500]{GeV}$ & $\unit[1000]{GeV}$ & $\unit[1500]{GeV}$\\
\hline
$\unit[1000]{GeV}$ & $5257_{\pm 24.6\% \ignore{4196}\pm 1.2\% \ignore{5163}}^{\pm 28.4\% \ignore{6714}\pm 1.8\% \ignore{5319}}$ 
& $340.4_{\pm 28.4\% \ignore{265}\pm 1.9\% \ignore{334}}^{\pm 31.0\% \ignore{446} \pm 1.6\% \ignore{346}}$ 
& $33.97_{\pm 30.6\% \ignore{26.0}\pm 1.2\%\ignore{33.5}}^{\pm 33.6\% \ignore{45.4}\pm 2.3\%\ignore{34.7}}$\\
$\unit[1500]{GeV}$ & $3263_{\pm 25.4\% \ignore{2602}\pm1.8\%\ignore{3205}}^{\pm 27.9\%\ignore{4172}\pm 1.2\%\ignore{3301}}$ 
& $260.7_{\pm 28.4\%\ignore{203}\pm 2.2\%\ignore{255}}^{\pm 30.8\% \ignore{341}\pm 1.3\% \ignore{264}}$ 
& $30.46_{\pm 30.7\%\ignore{23.3} \pm 1.9\%\ignore{29.9}}^{\pm  33.2\% \ignore{40.6} \pm 1.8\% \ignore{31.0}}$\\
$\unit[2000]{GeV}$ & $2179_{\pm 25.6\% \ignore{1735}\pm 1.9\%\ignore{2139}}^{\pm 27.9\% \ignore{2788}\pm 1.1\%\ignore{2204}}$ 
& $198.1^{\pm 30.7\%\ignore{259} \pm 1.5\%\ignore{201}}_{\pm 28.6\% \ignore{154}\pm 2.1\%\ignore{194}}$ 
& $25.75^{\pm 33.2\%\ignore{34.3}\pm 1.5\%\ignore{26.2}}_{\pm 30.7\% \ignore{19.7}\pm 2.0\%\ignore{25.3}}$\\
\end{tabular}
\caption{MSSM The uncertainty of the numerical integration is lower than one permille.}
\end{center}
\end{table}
