...
f_Tree = open('data/MSSM_ulur_LO_tree_msg=2000GeV.txt', 'r')
...
f_1L = open('data/MSSM_1L_ulur_msg=2000GeV.txt', 'r')
for lines in f_1L.readlines():
    discard = np.append(m_gluino, float(lines.split()[0]))
    sigma_NLO_1L_lr = np.append(sigma_NLO_1L_lr, float(lines.split()[1]))    
f_1L.close()
f_1L = open('data/MSSM_1L_ulul_msg=2000GeV.txt', 'r')
for lines in f_1L.readlines():
    discard = np.append(m_gluino, float(lines.split()[0]))
...
plt.ylabel(r"$K(uu \to \tilde{u}_L\tilde{u}_R)$ ", size = font_size)
...
plt.xlabel(r"$m_{\tilde{q}}$ in GeV", size = font_size)
plt.ylabel(r"$K(uu \to \tilde{u}_L\tilde{u}_L)$ ", size = font_size)
plt.xticks(fontsize = font_size)
plt.yticks(fontsize = font_size)
...
plt.xlabel(r"$m_{\tilde{q}}$ in GeV", size = font_size)
plt.ylabel(r"$K(uu \to \tilde{u}\tilde{u})$ ", size = font_size)
plt.xticks(fontsize = font_size)
plt.yticks(fontsize = font_size)
...
third_window.text(1400, 0.5, r"$m_{\tilde{g}} = 2000$ GeV", fontsize=font_size,
